...
class block :
    ...
    def __init__(self, block_dict):
        self.index = block_dict.get('index')
        self.data = block_dict.get('data')
        self.timeStamp = block_dict.get('timeStamp')
        self._prevHash = block_dict.get('_prevHash')

        if block_dict.get('_hash') is not None :
            self._hash = block_dict.get('_hash')
        else :
            self._hash = str(sha256(json.dumps({'index': self.index,
                                                'data': self.data,
